...
class ObjectDetection {
  /**
     ...
     */
  convertRgbaToGrayscale (src, dst) {
  ...
  /**
     ...
     */
  rescaleImage (src, srcWidth, srcHeight, factor, dst) {
  ...
  /**
     ...
     * @return {Array} 1-channel destination image
     */
  mirrorImage (src, srcWidth, srcHeight, dst) {
  ...
  /**
     ...
     * @return {Array} 1-channel destination image
     */
  computeCanny (src, srcWidth, srcHeight, dst) {
  ...
  /**
     ...
     * @return {Array} 1-channel destination image
     */
  equalizeHistogram (src, step, dst) {
  ...
  /**
     ...
     */
  computeSat (src, srcWidth, srcHeight, dst) {
  ...
  /**
     ...
     * @return {Uint32Array} 1-channel destination image
     */
  computeRsat (src, srcWidth, srcHeight, dst) {
    var dstWidth = srcWidth + 1

    var srcHeightTimesDstWidth = srcHeight * dstWidth
  ...
  /**
     ...
     * @return {Uint32Array} 1-channel destination image
     */
  computeSquaredSat (src, srcWidth, srcHeight, dst) {
    var dstWidth = srcWidth + 1
  ...
  /**
     ...
     */
  compileClassifier (src, width, scale = null, dst = null) {
    width += 1
  ...
  /**
     ...
     */
  detect (sat, rsat, ssat, cannySat, width, height, step, classifier) {
    width += 1
    height += 1

    var classifierUint32 = new Uint32Array(classifier.buffer)

    var windowWidth = classifierUint32[0]
          ...
          for (var j = 0, jEnd = classifierUint32[++i]; j < jEnd; ++j) {
            ...
            if (classifierUint32[++i]) {
              ...
              for (var kEnd = i + classifierUint32[++i]; i < kEnd;) {
                ...
                var packed = classifierUint32[++i]

                var f2 = f1 + (packed & 0xFFFF)

                var f3 = f1 + (packed >> 16 & 0xFFFF)
  ...
  /**
     ...
     */
  groupRectangles (rects, minNeighbors, confluence) {
    ...
    var abs = Math.abs; var min = Math.min
    for (var i = 0; i < rectsLength; ++i) {
      var found = false
      for (var j = 0; j < i; ++j) {
...
export default class Detector extends ObjectDetection {
  /**
  ...
  constructor (width, height, scaleFactor, classifier) {
    ...
    for (var i = 0; i < this.numScales; ++i) {
      ...
      this.compiledClassifiers[i] = super.compileClassifier(classifier, scaledWidth)
      scale *= scaleFactor
    }
  }
  /**
  ...
  detect (image, group, stepSize, roi, canny) {
    ...
    if (roi) { this.context.drawImage(image, roi[0], roi[1], roi[2], roi[3], 0, 0, width, height) } else { this.context.drawImage(image, 0, 0, width, height) }
    var imageData = this.context.getImageData(0, 0, width, height).data
    this.gray = super.convertRgbaToGrayscale(imageData, this.gray)
    ...
    for (var i = 0; i < this.numScales; ++i) {
      ...
      if (scale === 1) {
        ...
        this.scaledGray = super.rescaleImage(this.gray, width, height, scale, this.scaledGray)
      }

      if (canny) {
        this.canny = super.computeCanny(this.scaledGray, scaledWidth, scaledHeight, this.canny)
        this.cannySat = super.computeSat(this.canny, scaledWidth, scaledHeight, this.cannySat)
      }

      this.sat = super.computeSat(this.scaledGray, scaledWidth, scaledHeight, this.sat)
      this.ssat = super.computeSquaredSat(this.scaledGray, scaledWidth, scaledHeight, this.ssat)
      if (this.tilted) this.rsat = super.computeRsat(this.scaledGray, scaledWidth, scaledHeight, this.rsat)

      var newRects = super.detect(this.sat, this.rsat, this.ssat, this.cannySat, scaledWidth, scaledHeight, stepSize, this.compiledClassifiers[i])
    ...
    return (group ? super.groupRectangles(rects, group) : rects).sort(function (r1, r2) { return r2[4] - r1[4] })
  }
}
