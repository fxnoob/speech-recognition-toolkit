...
self.addEventListener(
  ...
  evt => {
    ...
    if (action == "emoji") {
      ...
      self.postMessage({ langId, emojiName, uid, action, emoji });
    } else if (action == "emoji_list") {
      const emojiJson = getEmojiList(langId);
      const emojiArrayFormatted = Object.keys(emojiJson).map(key => {
        return [key, emojiJson[key].replacement];
      });
      self.postMessage({ langId, uid, action, emojiList: emojiArrayFormatted });
    }
  },
  false
);
