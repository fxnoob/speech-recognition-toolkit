...
export class speechTranslation extends speechRecognition {
  constructor () {
    super();
    this.yr = null;
  }
  getCode(desiredLang) {
    const langs = this.langExt;
    if (!desiredLang) {
      return false;
    }
  ...
  sM(a) {
    let b;
    if (null !== this.yr) b = this.yr;
    else {
      b = this.wr(String.fromCharCode(84));
      var c = this.wr(String.fromCharCode(75));
      b = [b(), b()];
      b[1] = c();
      b = (this.yr = window[b.join(c())] || "") || "";
    }
    var d = this.wr(String.fromCharCode(116)),
      c = this.wr(String.fromCharCode(107)),
      d = [d(), d()];
    d[1] = c();
    c = "&" + d.join("") + "=";
    d = b.split(".");
    ...
    a = b;
    for (f = 0; f < e.length; f++) (a += e[f]), (a = this.xr(a, "+-a^+6"));
    a = this.xr(a, "+-3^+b+-f");
    a ^= Number(d[1]) || 0;
  ...
  translate(text, opts) {
    ...
    opts.to = opts.to || "en";
    opts.from = this.getCode(opts.from);
    opts.to = this.getCode(opts.to);
